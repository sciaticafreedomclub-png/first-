...
function Label({ nodeIndex, start, place, children }) {
  const t = useTime();
  const dur = 0.6;
    ...
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', background: '#0E0E0E' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <Label nodeIndex={0} start={0.45} place="above">Cut out all caffeine</Label>
          <Label nodeIndex={1} start={0.9} place="below">Stick to just 2 routine habits</Label>
          <Label nodeIndex={2} start={1.2} place="above">Optimize your sleep</Label>
          <Label nodeIndex={3} start={1.5} place="below">Stop just resting</Label>
          <Label nodeIndex={4} start={1.8} place="above">Stick to only a few exercises</Label>
        </div>
      </div>
...
function App() {
  return (
    <Stage width={W} height={H} duration={DUR} fps={24} background="#0E0E0E" persistKey="sfc-teaser5b">
      <Scene />
    </Stage>
  );
}
