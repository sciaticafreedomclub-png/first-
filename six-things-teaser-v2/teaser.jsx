...
function Numeral({ i, node }) {
  ...
  const pulse = gold ? 0.5 + 0.5 * Math.sin((t - 4) * 2.3) : 0;
...
function Scene() {
  ...
  const payoff = Easing.easeOutSine(clamp((t - 3.9) / 1.7, 0, 1));
...
function App() {
  return (
    <Stage width={W} height={H} duration={DUR} fps={24} background="transparent" persistKey="sfc-teaser">
      <Scene />
    </Stage>
  );
}
