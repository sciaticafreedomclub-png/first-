...
function Label({ nodeIndex, start, place, children, restBlur = 0 }) {
  const t = useTime();
  const dur = 0.6;
  ...
  return (
    <div style={{
      ...
      opacity: e,
      filter: `blur(${(1 - e) * 10 + restBlur}px)`,
      whiteSpace: 'nowrap',
      willChange: 'transform, opacity',
    }}>
      <div style={{
    ...
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', background: '#0E0E0E' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <Label nodeIndex={0} start={0.45} place="above" restBlur={caffeineBlur}>Cut out all caffeine</Label>
        </div>
      </div>
  ...
  return (
    <React.Fragment>
      <Stage width={W} height={H} duration={DUR} fps={24} background="#0E0E0E" persistKey="sfc-teaser">
        <Scene caffeineBlur={tw.caffeineBlur} />
      </Stage>
      <TweaksPanel>
        <TweakSection label="Caffeine label" />
        <TweakSlider label="Blur" value={tw.caffeineBlur} min={0} max={48} step={1} unit="px"
                     onChange={(v) => setTweak('caffeineBlur', v)} />
      </TweaksPanel>
    </React.Fragment>
  );
}
