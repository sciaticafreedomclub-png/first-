// render.js — render Simple System.html to MP4 via Playwright + ffmpeg
...
function startServer(port = 3972) {
  const mime = {
...
const FPS = 24;
const DURATION = 6;
const TOTAL_FRAMES = FPS * DURATION; // 144
const WIDTH = 1920;
const HEIGHT = 1080;
...
(async () => {
  ...
  // Pause and hide UI chrome
  await page.evaluate(() => {
    if (window.__stageCtrl) window.__stageCtrl.setPlaying(false);
    // Hide playback bar (last child of stage)
    ...
    // Hide tweaks panel if present
    document.querySelectorAll('[data-tweaks-panel]').forEach(el => el.style.display = 'none');
  });
  ...
  const outputPath = path.join(__dirname, 'simple_system.mp4');
  execSync(
