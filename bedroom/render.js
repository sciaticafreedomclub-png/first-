...
function startServer(port = 3976) {
...
const FPS = 24;
const DURATION = 10.2;
const TOTAL_FRAMES = Math.ceil(FPS * DURATION); // 245
const WIDTH = 1920;
const HEIGHT = 1080;
...
(async () => {
  ...
  for (let frame = 0; frame < TOTAL_FRAMES; frame++) {
    ...
    if (frame % 24 === 0) process.stdout.write(`\r  frame ${frame + 1}/${TOTAL_FRAMES} (t=${(t).toFixed(1)}s)`);
  }
  ...
  const outputPath = path.join(__dirname, 'bedroom_for_sleeping.mp4');
  execSync(
