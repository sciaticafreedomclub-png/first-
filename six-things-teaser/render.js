...
function startServer(port = 3973) {
...
(async () => {
  ...
  for (let frame = 0; frame < TOTAL_FRAMES; frame++) {
    ...
    await page.screenshot({ path: path.join(FRAMES_DIR, `frame_${String(frame).padStart(4, '0')}.png`), clip: { x: 0, y: 0, width: WIDTH, height: HEIGHT } });
  ...
  const outputPath = path.join(__dirname, 'six_things_teaser.mp4');
  execSync(
