...
// number 6 is the only thing that animates in; everything else starts shown
const SIX_START = 0.4;
const SIX_DUR = 0.75;
function sixEase(t) {
  return Easing.easeOutCubic(clamp((t - SIX_START) / SIX_DUR, 0, 1));
}

function nodeEase(i, t) {
  if (i < 5) return 1;            // steps 1–5 are visible from the first frame
  return sixEase(t);
}
...
// A caption pinned to a numeral, either above or below it.
function Label({ nodeIndex, start, place, children, width }) {
  const t = useTime();
  const dur = 0.6;
...
function Thread() {
  const t = useTime();
  // thread up to step 5 is drawn from the start; only the final segment to 6 animates
  const draw = interpolate([SIX_START, SIX_START + SIX_DUR], [0.8, 1.0], Easing.easeInOutCubic)(t);
  return (
    <svg
...
function Scene() {
  ...
  // already zoomed in close on number 6 from the very start
  const camScale = 1.25;
    ...
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', background: '#0E0E0E' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <Label nodeIndex={0} start={-1} place="above">Cut out all caffeine</Label>
          <Label nodeIndex={1} start={-1} place="below">Stick to just 2 routine habits</Label>
          <Label nodeIndex={2} start={-1} place="above">Optimize your sleep</Label>
          <Label nodeIndex={3} start={-1} place="below">Stop just resting</Label>
          <Label nodeIndex={4} start={-1} place="above">Stick to only a few exercises</Label>
          <Label nodeIndex={5} start={SIX_START + 0.15} place="below" width={1100}>Don't do back extension strengthening every day</Label>
        </div>
      </div>
...
function App() {
  return (
    <Stage width={W} height={H} duration={DUR} fps={24} background="#0E0E0E" persistKey="sfc-teaser6">
      <Scene />
    </Stage>
  );
}
